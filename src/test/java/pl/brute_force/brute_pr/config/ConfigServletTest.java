...
public class ConfigServletTest {
  ...
  public void testFoundRepo() throws Exception {
    sut.handleRequest("/plugins/servlet/brute-pr/PRJ/repo1", "user1", response);
  ...
  public void testFoundRepo_differentContext() throws Exception {
    sut.handleRequest("/bitbucket/plugins/servlet/brute-pr/PRJ/repo1", "user1", response);
    verify(response, times(1)).setStatus(HttpServletResponse.SC_OK);
    verify(renderer, times(1)).render(anyString(), any(Map.class), any(Writer.class));
  }
  ...
  public void testHandleMalformedRequest() throws Exception {
    sut.handleRequest("/plugins/servlet/saywhat/brute-pr/PRJ/repo1", "user1", response);
  ...
  public void testHandleMissingRepo() throws Exception {
    sut.handleRequest("/plugins/servlet/brute-pr/FAKE/repo1", "user1", response);
    verify(response, atLeastOnce()).setStatus(HttpServletResponse.SC_NOT_FOUND);
  }

  @Test
  public void testPermissionDenied() throws Exception {
    sut.handleRequest("/plugins/servlet/brute-pr/PRJ/repo1", "nonuser1", response);
